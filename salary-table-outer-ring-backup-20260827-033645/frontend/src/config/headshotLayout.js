...
export const HEADSHOT_LAYOUTS = {
  ...
  "all-nba":            { x: 0, y: 18, scale: 1 },
  "all-rookie":         { x: 0, y: 17.5, scale: 1 },
  "all-defensive":      { x: 0, y: 18, scale: 1 },
  ...
  "salary-table": {
    // ONLY EDIT THESE Salary Table numbers.
    headshot: { x: 400, y: 200, scale: 1.9 },
    overall:  { x: 30, y: 0,   scale: 1.1 },
    name:     { x: 50, y: 0,   scale: 1 },
    option:   { x: 50, y: 0,   scale: 1 },
    rowHeight: 60,
  },
...
export function getHeadshotLayout(pageKey = "") {
  ...
  const layout = HEADSHOT_LAYOUTS[key] || { x: 0, y: 0, scale: 1 };
  // Salary Table keeps all of its manual controls together in HEADSHOT_LAYOUTS.
  // Other pages continue using the original flat { x, y, scale } format.
  const row = layout?.headshot || layout;
