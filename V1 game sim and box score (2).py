...
# --- Load JSON ---
with open("atlantic + northwest (6).json", encoding="utf-8") as f:
    data = json.load(f)
...
# ===================== NEW GAME SCORING CALIBRATION (season targets) =====================
# Targets:
#   OFF=98 vs avg DEF → ~122 PPG
#   OFF=65 vs avg DEF → ~105 PPG
#   DEF=98 vs avg OFF → ~105 allowed
#   DEF=65 vs avg OFF → ~121 allowed
# League "means" around 80.
OFF_MEAN = 80.0
DEF_MEAN = 80.0
BASE_O   = 112.7272727273                # baseline so OFF endpoints hit with coefficients
OFF_COEF = 17.0/33.0                     # ~0.51515 per OFF point vs mean
DEF_COEF = 16.0/33.0                     # ~0.48485 per DEF point vs mean (subtract)
DEF_BIAS = 1.0                           # tiny bias to nail both DEF endpoints

# Pace/total shaping by offensive/defensive mix:
PACE_A   = 0.0042                        # offense raises pace/total
PACE_D   = 0.0032                        # defense lowers pace/total
PACE_CLAMP = (0.92, 1.08)                # keep totals realistic

# Outcome (win prob) driven by OVERALL only; set ~80/20 at ΔOVR=24 (e.g., OKC vs BKN).
def sigma_margin_for_delta(d):
    # moderate variance; slightly smaller in bigger mismatches
    return max(7.0, min(13.0, 13.0 - 0.25 * abs(d)))

def sigma_total_for_delta(d):
    # total variance modestly smaller in mismatches
    return max(10.0, min(16.0, 16.0 - 0.10 * abs(d)))

# Choose margin-per-OVR so that Φ( (Δ * K)/σ ) ≈ 0.80 when |Δ|=24, σ≈7 → K≈0.2455
MARG_PER_OVR = 0.8416 * 7.0 / 24.0       # ≈0.2455
...
class TeamPanel(tk.Frame):
    def __init__(self, parent, label, on_update=None):
        ...
        # store a copy of players per team selection
        self.team_data = {}
    ...
    def autocomplete_rotation(self):
        ...
        # ---- original choose ~10 and assign minutes (UNCHANGED) ----
        ...
        # ---- NEW: choose starters by MAX AVERAGE OVR, primary preferred; DO NOT change minutes ----
        # Score = mean OVR + small bonus for primary matches, tiny penalty for secondary
        PRIMARY_BONUS = 0.02   # adds up to ~0.10 if all 5 are primaries; doesn't override OVR
        SECONDARY_PEN = 0.01   # light nudge against secondary

        best_mapping = None
        best_score = -1e9

        # generate all 5-man combos & position assignments that respect primary/secondary eligibility
        for combo in itertools.combinations(chosen, 5):
            # Precompute each player's eligible slots (primary always allowed; secondary allowed too)
            elig = {}
            for pl in combo:
                slots = set()
            ...
            # Try all position permutations with constraint checking
            for perm in itertools.permutations(POSITIONS):
                ...
                for player, slot in zip(combo, perm):
                    # must be eligible (primary or secondary)
        ...
        # Fallback (shouldn't happen): top-5 by OVR into PG..C in order
        if not best_mapping:
            lineup = sorted(chosen, key=lambda x: x["overall"], reverse=True)[:5]
            best_mapping = {pos: ply for pos, ply in zip(POSITIONS, lineup)}

        # Build final ordering: starters first (PG..C), then bench sorted by minutes DESC.
        ...
        # zero out minutes for non-chosen
        for p in self.players:
            if p not in chosen:
                p["minutes"] = 0

        # reorder roster for display; MINUTES REMAIN EXACTLY AS ALLOCATED ABOVE
...
# ---------- MAIN ----------
root = tk.Tk()
root.title("Coach Gameplan – Home vs Away")
...
home = TeamPanel(root, "Home", on_update=update_sim_state)
center = tk.Frame(root, bg="#f5f5f5", width=280, bd=2, relief="solid")
center.pack(side="left", fill="y", padx=6, pady=6)
away = TeamPanel(root, "Away", on_update=update_sim_state)
...
def generate_boxscore(team_obj, team_points, team_ratings, num_ot_periods):
    ...
    # ----- Team strengths (minute-weighted, active only) -----
    ...
    # ----- Player-level 3PA -----
    LOW_3PT_HARD_CAP = 50  # <50 → never attempt a three
    ...
    per_min_cap = []
    for i, p in enumerate(plist):
        cap = int(round((p["minutes"]/36.0) * (4.5 + 0.03*(p["offRating"] - 75)))) + (1 if i in top3_idx else 0)
        cap += random.choice([0,0,1])
        per_min_cap.append(max(0, cap))
    overflow = 0
    for i in range(len(fta)):
        if fta[i] > per_min_cap[i]:
            overflow += fta[i] - per_min_cap[i]
            fta[i] = per_min_cap[i]
    if overflow > 0:
        order = sorted(range(len(plist)), key=lambda j: (ft_weights[j], per_min_cap[j]-fta[j]), reverse=True)
        for j in order:
            if overflow <= 0: break
            room = max(0, per_min_cap[j]-fta[j])
            add = min(room, overflow)
            if add > 0: fta[j] += add; overflow -= add

    # ----- Percentages (tighter FT variance) -----
    def pct_two_for(p):
    ...
    touches = [fga[i] + 0.44*fta[i] + 0.30*asts[i] for i in range(len(plist))]
    def poisson(lmbd):
    ...
    # ----- Personal fouls with per-player cap 6 -----
    ...
    if P0 != target:
        ...
        for arr, att in ((two_made, two_att), (three_made, three_att)):
        ...
        def bump_up():
            for i in top3_idx + [k for k in range(len(plist)) if k not in top3_idx]:
                if three_made[i] < three_att[i]: three_made[i]+=1; return 3
            for i in top3_idx + [k for k in range(len(plist)) if k not in top3_idx]:
                if two_made[i] < two_att[i]: two_made[i]+=1; return 2
            for i in top3_idx + [k for k in range(len(plist)) if k not in top3_idx]:
                if ft_made[i] < min(fta[i], int(0.9*fta[i])): ft_made[i]+=1; return 1
            return 0
...
# ----------- Simulate Game (uses new calibrated scoring) -----------
def simulate_game():
    # Ensure latest slider values are saved
    ...
    # 1) OVERALL drives win probability / margin
    d_ovr = h["overall"] - a["overall"]
    sig_m = sigma_margin_for_delta(d_ovr)
    sig_t = sigma_total_for_delta(d_ovr)
    margin_mean = MARG_PER_OVR * d_ovr

    # 2) OFF/DEF drive scoring style (total/pace)
    ...
    mu_home = base_home * pace
    mu_away = base_away * pace
    total_mean = mu_home + mu_away

    # 3) Sample total and margin with moderate randomness
    sampled_total  = random.gauss(total_mean, sig_t)
    sampled_margin = random.gauss(margin_mean, sig_m)

    # 4) Convert to team scores
    hs = int(round((sampled_total + sampled_margin) / 2.0))
    as_ = int(round(sampled_total - hs))

    # Clamp to NBA-ish range without forcing blowouts
    hs = clamp(hs, 85, 155)
    as_ = clamp(as_, 85, 155)
    ...
    ot_h = ot_a = 0
    # Small OT chance if tied after regulation
    if sum(hq) == sum(aq):
    ...
    # ---------- Build box scores that match the final ----------
...
# --- Winner label and simulate button ---
