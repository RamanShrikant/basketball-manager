...
def _is_not_started_draft_state(state: Any) -> bool:
    if not isinstance(state, dict):
        return False
    return (
        not bool(state.get("completed"))
        and _safe_int(state.get("currentPickIndex"), 0) <= 0
        and not bool(state.get("draftedPicks") or [])
    )


def _is_stale_generated_auto_class_meta(class_meta: Any) -> bool:
    if not isinstance(class_meta, dict):
        return False

    # PATCH50: any already-created auto class from the old inflated generator
    # should be rebuilt before the draft starts, even if it was created with a
    # fresh random seed. Custom/provided classes do not carry this ratingModel.
    rating_model = str(class_meta.get("ratingModel") or "").strip()
    if rating_model and rating_model != EXPECTED_AUTOGEN_DRAFT_RATING_MODEL:
        return True

    seed_mode = str(class_meta.get("seedMode") or "").strip().lower()
    if seed_mode == "fresh_random":
        return False

    # Old generated classes either carried seedMode="provided" because
    # draft_logic forced a season-year seed, or carried no seedMode at all but
    # still had a generated-class summary/seed/classType. Custom/provided draft
    # classes should not be treated as stale here.
    if seed_mode == "provided":
        return True

    summary = str(class_meta.get("summary") or "").strip().lower()
    has_generated_summary = "generated" in summary and "draft class" in summary
    has_seed = class_meta.get("seed") is not None
    has_class_type = bool(class_meta.get("classType"))

    return bool(has_generated_summary and (has_seed or has_class_type))


def _is_stale_generated_auto_draft_state(state: Any) -> bool:
    if not _is_not_started_draft_state(state):
        return False
    return _is_stale_generated_auto_class_meta((state or {}).get("classMeta") or {})
...
def initialize_draft(league_data: Dict[str, Any], payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    existing_state_is_stale_auto = _is_stale_generated_auto_draft_state(existing_state)
    if (
        ...
        and not existing_state_is_stale_auto
    ):
        return {
            "ok": True,
    ...
    draft_state_root = league.get("draftState") if isinstance(league.get("draftState"), dict) else {}
    draft_state_blob = draft_state_root.get("draft") if isinstance(draft_state_root.get("draft"), dict) else {}
    draft_class = payload.get("draftClass") or draft_state_root.get("draftClass")

    if (
        payload.get("draftClass") is None
        and isinstance(draft_class, list)
        and _is_stale_generated_auto_class_meta(draft_state_blob.get("classMeta") or {})
    ):
        draft_class = None
    ...
    if not isinstance(draft_class, list) or len(draft_class) < len(draft_order):
        ...
        avoid_names = []
        for active_player in _league_players_for_v19_baselines(league):
            name = active_player.get("name") or active_player.get("playerName") or active_player.get("player")
            if name:
                avoid_names.append(str(name))
        for active_player in league.get("freeAgents") or []:
        ...
        # Important: do NOT pass a season-year fallback seed here. If the caller
        # did not explicitly provide classSeed, let autogenerated_draft_class.py
        # create a fresh random seed. Passing _stable_seed("draft_class", year)
        # recreates the exact old Rakai Hayashi class every time.
        if class_seed is not None:
            generator_kwargs["seed"] = _safe_int(class_seed, _stable_seed("draft_class", season_year))

        generated = generate_draft_class(**generator_kwargs)
