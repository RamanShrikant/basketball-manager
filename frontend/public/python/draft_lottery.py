...
def _record_sort_key_worst_first(row: Dict[str, Any]) -> Tuple[float, int, int, str]:
    wins = _safe_int(row.get("wins"), 0)
    losses = _safe_int(row.get("losses"), 0)
    games = wins + losses
    win_pct = wins / games if games else 0.0
    league_rank = _safe_int(row.get("leagueRank"), 0)
    # leagueRank is generated by the canonical standings helper where 1 is best
    # and 30 is worst. Use it as the tiebreak source so lottery/draft order does
    # not silently disagree with Standings/Playoff seeding on equal records.
    if league_rank > 0:
        return (-league_rank, win_pct, _safe_int(row.get("pointDifferential"), 0), _team_name(row))
    return (0.0, win_pct, _safe_int(row.get("pointDifferential"), 0), _team_name(row))


def _record_sort_key_best_first(row: Dict[str, Any]) -> Tuple[float, float, int, str]:
    wins = _safe_int(row.get("wins"), 0)
    losses = _safe_int(row.get("losses"), 0)
    games = wins + losses
    win_pct = wins / games if games else 0.0
    ...
    if league_rank > 0:
        return (league_rank, -win_pct, -_safe_int(row.get("pointDifferential"), 0), _team_name(row))
    return (999.0, -win_pct, -_safe_int(row.get("pointDifferential"), 0), _team_name(row))
