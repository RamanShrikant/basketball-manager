...
// ------------------------------------------------------------
// DRAFT LOTTERY REQUEST MODE
// ------------------------------------------------------------
async function runDraftLotteryRequest(requestId, leagueData, payload) {
  try {
    pyodide.globals.set("draft_lottery_request_js", pyodide.toPy({
      action: "run_draft_lottery",
      leagueData: leagueData || {},
      payload: payload || {},
    }));
    ...
    const pyJson = await pyodide.runPythonAsync(`
import importlib, json
import draft_lottery
importlib.reload(draft_lottery)
from draft_lottery import handle_request
res = handle_request(draft_lottery_request_js)
json.dumps(res)
    `);

    postMessage({
      type: "draft-lottery-result",
      requestId,
      payload: JSON.parse(pyJson),
    });
  } catch (err) {
    console.error("[simWorkerV2] draft lottery error:", err);
    postMessage({
      type: "draft-lottery-error",
      requestId,
      error: err.toString(),
    });
  }
...
// ------------------------------------------------------------
// DRAFT LOGIC REQUEST MODE
// ------------------------------------------------------------
async function runDraftLogicRequest(requestId, action, leagueData, payload) {
  try {
    pyodide.globals.set("draft_request_js", pyodide.toPy({
      action,
      leagueData: leagueData || {},
      payload: payload || {},
    }));
    ...
    const pyJson = await pyodide.runPythonAsync(`
import importlib, json
import autogenerated_draft_class
import draft_logic
importlib.reload(autogenerated_draft_class)
importlib.reload(draft_logic)
from draft_logic import handle_request
res = handle_request(draft_request_js)
json.dumps(res)
    `);

    postMessage({
      type: "draft-action-result",
      requestId,
      payload: JSON.parse(pyJson),
    });
  } catch (err) {
    console.error("[simWorkerV2] draft logic error:", action, err);
    postMessage({
      type: "draft-action-error",
      requestId,
      error: err.toString(),
    });
  }
...
// ------------------------------------------------------------
// TEAM ROSTER / ROOKIE SIGNINGS REQUEST MODE
// ------------------------------------------------------------
async function runTeamRosterLogicRequest(requestId, action, leagueData, payload) {
  try {
    pyodide.globals.set("team_roster_request_js", pyodide.toPy({
      action,
      leagueData: leagueData || {},
      payload: payload || {},
    }));
    ...
    const pyJson = await pyodide.runPythonAsync(`
import importlib, json
import team_roster_logic
importlib.reload(team_roster_logic)
from team_roster_logic import handle_request
res = handle_request(team_roster_request_js)
json.dumps(res)
    `);

    postMessage({
      type: "team-roster-action-result",
      requestId,
      payload: JSON.parse(pyJson),
    });
  } catch (err) {
    console.error("[simWorkerV2] team roster logic error:", action, err);
    postMessage({
      type: "team-roster-action-error",
      requestId,
      error: err.toString(),
    });
  }
...
onmessage = async (e) => {
  ...
  if (msg.type === "run-draft-lottery") {
    const leaguePayload = msg.leagueData ?? msg.league ?? {};
    return runDraftLotteryRequest(msg.requestId, leaguePayload, msg.payload || {});
  }

  if (msg.type === "run-draft-action") {
    const leaguePayload = msg.leagueData ?? msg.league ?? {};
    return runDraftLogicRequest(msg.requestId, msg.action, leaguePayload, msg.payload || {});
  }

  if (msg.type === "run-team-roster-action") {
    const leaguePayload = msg.leagueData ?? msg.league ?? {};
    return runTeamRosterLogicRequest(msg.requestId, msg.action, leaguePayload, msg.payload || {});
  }
