...
// config/headshotLayout.js under SALARY_TABLE_VISUAL_TUNING.
...
function useSalaryTableManualVisualTuning() {
  const readViewportWidth = () => {
    if (typeof window === "undefined") return Number(SALARY_TABLE_VISUAL_TUNING.referenceWidth || 1600);
  ...
  const responsiveScale = getResponsiveVisualScale(viewportWidth, SALARY_TABLE_VISUAL_TUNING);
  const rowHeight = Math.max(1, Number(SALARY_TABLE_VISUAL_TUNING?.row?.height || 60)) * responsiveScale;

  const resolveVisual = (key) => {
    const visual = SALARY_TABLE_VISUAL_TUNING?.[key] || {};
