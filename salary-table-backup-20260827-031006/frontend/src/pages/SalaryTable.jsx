...
// SALARY TABLE PLAYER VISUAL TUNING — manual VS Code controls.
// Laptop mode is automatic at <= 1440px so the portrait + OVR/POT ring stay
// compact enough for the full salary grid without sacrificing the existing
// horizontal-scroll fallback on very small windows.
...
export default function SalaryTable() {
  ...
  const renderSalaryPlayerIdentity = (p, teamName) => (
    <div className="flex min-w-0 items-center" style={{ gap: salaryPlayerTuning.gap }}>
      ...
      <div
        className="shrink-0"
        style={{ transform: `translate(${salaryPlayerTuning.ring.x}px, ${salaryPlayerTuning.ring.y}px)` }}
      >
        <PlayerRatingRing
          overall={p.overall}
      ...
      <div className="min-w-0 leading-tight">
        <div className="flex min-w-0 items-center gap-2 font-semibold" style={{ fontSize: salaryPlayerTuning.nameSize }}>
        ...
        <div className={p.isDeadCap || p.isCapHold ? "mt-0.5 truncate text-xs text-red-200/75" : "mt-0.5 truncate text-xs text-white/50"}>
          {p.optionLabel}
        </div>
      </div>
    </div>
