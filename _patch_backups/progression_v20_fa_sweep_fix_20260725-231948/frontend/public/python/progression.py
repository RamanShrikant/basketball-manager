...
PROGRESSION_PY_VERSION = "2026-07-25_progression_v20_fine_shape_potential_momentum"
...
DEFAULT_SETTINGS: Dict[str, Any] = {
    ...
    "progression": {
        ...
        # v20 uses the active roster file as the baseline, with fine-grained
        # cumulative shelves plus exact-rung smoothing so players do not clump
        # at a few OVR values while still allowing new stars to emerge.
    ...
    "potential_update": {
        ...
        # Season-production impact on potential. Strong years help keep/open a
        # ceiling; weak years can close it slowly after repeated stalls.
        "young_performance_signal": 0.300,
        "mid_performance_signal": 0.220,
        "late_performance_signal": 0.120,
...
def _predict_dynamic_potential_after_progression(
    ...
    perf = _performance_signal(stats, new_age, new_overall)
    if new_age <= 22:
        perf_signal = float(cfg.get("young_performance_signal", 0.30))
    elif new_age <= 25:
        perf_signal = float(cfg.get("mid_performance_signal", 0.22))
    else:
        perf_signal = float(cfg.get("late_performance_signal", 0.12))
    raw_delta += perf * perf_signal
    ...
    # v20: breakouts can reopen ceilings. Potential is a strong guide, not a
    # permanent prison; this is what allows occasional surprise new stars.
    low_ceiling_breakout = bool(new_age <= 27 and ovr_delta >= 2 and old_potential <= old_overall + 1)
    late_bloomer_breakout = bool(24 <= new_age <= 28 and ovr_delta >= 2 and old_potential <= new_overall + 1)
    if low_ceiling_breakout and pot_delta < 1:
        pot_delta = 1
    if late_bloomer_breakout and pot_delta < 1:
        pot_delta = 1
    ...
    if low_ceiling_breakout:
        max_allowed = max(max_allowed, min(99, new_overall + (4 if new_age <= 24 else 3)))
    if late_bloomer_breakout:
        max_allowed = max(max_allowed, min(99, new_overall + 2))
...
def _stat_context(stats: Optional[Dict[str, Any]], settings: Dict[str, Any], age: int = 25, overall: int = 75) -> Tuple[float, float]:
    """
    Return minutes and season-production nudges.

    v20 makes production a real part of the roll instead of a nearly invisible
    tiebreaker. It still cannot override age/POT/dev path or the final league
    shape lock by itself.
    """
    mpg = _extract_mpg(stats)
    min_fac = _minutes_factor(mpg, settings)
    prod_adj = _performance_signal(stats, age, overall)
...
def _development_momentum_adjustment(p: Dict[str, Any], age: int, overall: int, potential: int) -> float:
    """Persistent career-memory nudge used by v20 progression.

    Momentum is deliberately small. It does not guarantee careers, but it makes
    repeated breakouts/stalls matter so repeated saves produce believable arcs
    instead of one disconnected roll every offseason.
    """
    if age > 30:
        return 0.0

    state = _development_momentum_state(p)
    hot = _safe_int(state.get("hotYears"), 0)
    stalled = _safe_int(state.get("stalledYears"), 0)
    decline = _safe_int(state.get("declineYears"), 0)
    gap = max(0, _safe_int(potential, overall) - _safe_int(overall, 70))
    path = _player_dev_path_value(p)

    adj = 0.0
    if hot > 0 and gap > 0 and age <= 27:
        adj += min(0.28, 0.10 + 0.06 * hot)
    if stalled >= 2 and age <= 27:
        adj -= min(0.24, 0.07 * stalled)
    if decline >= 2 and age <= 29:
        adj -= min(0.28, 0.08 * decline)

    if path in {"ceiling_hit", "star"} and hot > 0 and gap > 0:
        adj += 0.06
    if path == "bust" and stalled > 0:
        adj -= 0.05
    if path == "late_bloomer" and 24 <= age <= 28 and hot > 0:
        adj += 0.08

    return _clamp(adj, -0.34, 0.38)
...
def _controlled_low_mid_delta(
    ...
    if team_name == "__FREE_AGENCY__":
        ...
        perf = _performance_signal(stats, age, overall)
        if perf >= 0.30 and delta <= 0 and age <= 27 and potential > overall and rng.random() < 0.28:
            delta += 1
        elif perf <= -0.28 and delta >= 1 and rng.random() < 0.25:
            delta -= 1
        return int(_clamp(delta, -5 if age >= 34 else -3, 3))
    ...
    # v20: real season production and career momentum can swing the controlled
    # low/mid table by one point. This keeps potential important without making
    # the tables deterministic.
    perf = _performance_signal(stats, age, overall)
    momentum_adj = _development_momentum_adjustment(p, age, overall, potential)
    positive_case = perf + momentum_adj
    negative_case = perf + momentum_adj
    if positive_case >= 0.34 and age <= 27 and potential > overall and delta <= 0:
        chance = 0.22 + (0.10 if level in {"strong", "elite"} else 0.0)
        if rng.random() < chance:
            delta += 1
    elif positive_case >= 0.48 and age <= 25 and potential >= overall + 3 and delta == 1:
        if rng.random() < (0.14 if level != "normal" else 0.07):
            delta += 1
    if negative_case <= -0.32 and delta >= 1:
        if rng.random() < 0.24:
            delta -= 1
    elif negative_case <= -0.46 and delta == 0 and potential <= overall + 2:
        if rng.random() < 0.18:
            delta -= 1
    ...
    if potential <= overall + 1 and delta > 0 and dev_path != "ceiling_hit" and rng.random() < 0.42:
        delta = max(0, delta - 1)

    if age >= 24 and overall < 77 and delta > 0 and level == "normal" and rng.random() < 0.24:
        delta -= 1
...
def _target_delta_for_player(
    ...
    expected += _development_momentum_adjustment(p, age, overall, potential)
...
def _build_progression_baseline_from_plan(plan: List[Dict[str, Any]], settings: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "version": "v20_fine_shelf_exact_rung_baseline",
...
def _apply_league_rating_governor(
    ...
    _apply_v20_fine_shape_lock(plan, baseline, settings, rng)
