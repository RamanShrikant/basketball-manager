...
PROGRESSION_PY_VERSION = "2026-07-25_progression_v20_fine_shape_no_stats_wide_paths"
...
def _predict_dynamic_potential_after_progression(
    ...
    # Do not use current-season box-score production to recalculate potential.
    # Potential should remain a probabilistic career guide, not a reward/punish
    # meter for one noisy sim year.
...
def _development_momentum_adjustment(p: Dict[str, Any], age: int, overall: int, potential: int) -> float:
    ...
    adj = 0.0
    # Keep momentum as a very small memory nudge only. It should never prevent a
    # late year-4/year-5 leap after early struggles, and it should never lock a
    # fast starter into permanent star growth.
    if hot > 0 and gap > 0 and age <= 27:
        adj += min(0.06, 0.025 + 0.010 * hot)
    if stalled >= 2 and age <= 27:
        adj -= min(0.055, 0.012 * stalled)
    if decline >= 2 and age <= 29:
        adj -= min(0.060, 0.014 * decline)
...
def _controlled_low_mid_delta(
    ...
    if team_name != "__FREE_AGENCY__":
        ...
        if dev_path == "ceiling_hit" and age <= 26 and gap > 0:
            if delta <= 0 and rng.random() < (0.16 if potential >= 95 else 0.08):
                delta += 1
            elif delta == 1 and potential >= 95 and overall >= 80 and rng.random() < 0.05:
                delta += 1
        elif dev_path == "star" and age <= 25 and gap > 0:
            if delta <= 0 and rng.random() < 0.07:
                delta += 1
        elif dev_path in {"good", "late_bloomer"} and age <= 25 and gap > 0:
            if delta <= -1 and rng.random() < 0.045:
                delta += 1
        elif dev_path == "bust" and age <= 25:
            if delta > 0 and rng.random() < 0.12:
                delta -= 1
        elif dev_path == "slow" and age <= 25:
            if delta > 1 and rng.random() < 0.55:
                delta -= 1

    # No season-performance or momentum override here. This keeps the outcome
    # table wide enough for late leaps, early peaks, stalls, and rebounds across
    # different saves instead of locking a player into last year's story.
